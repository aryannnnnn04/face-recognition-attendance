#!/usr/bin/env bash
# exit on error
set -o errexit

echo "--- Adding swap space in /var/tmp to handle large build ---"
fallocate -l 1G /var/tmp/swapfile
chmod 600 /var/tmp/swapfile
mkswap /var/tmp/swapfile
swapon /var/tmp/swapfile

echo "--- Upgrading build tools ---"
pip install --upgrade pip setuptools wheel

echo "--- Installing Python dependencies from requirements.txt ---"
pip install -r requirements.txt

echo "--- Build complete, removing swap space ---"
swapoff /var/tmp/swapfile
rm /var/tmp/swapfile
```

### Final Steps to Get Your App Live

Now, you just need to upload this final fix to GitHub and tell Render to try one last time.

1.  **Save the changes** to the `build.sh` file in VS Code.

2.  **Upload the fix to GitHub.** Open your terminal in VS Code and run these commands:
    ```bash
    git add .
    git commit -m "Fix build script path for swap file"
    git push
    

